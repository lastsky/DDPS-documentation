...
# Fastnetmon configuration parameters and files

The full fastnetmon configuration is stored in tree files:

  - ``/etc/fastnetmon.conf``: the configuration file - see below. Threshold values are set here.
  - ``/etc/networks_whitelist``: a list of _white listed networks_ which will not trigger alarms. The path may be changed in ``fastnetmon.conf``
  - ``/etc/networks_list``: all customer networks in CIDR format

Please notice the files below are _example files_.

## networks_list

List of customer networks visible to fastnetmon.
...
Networks which will not be filtered.
...
Some parameters are more interesting than others; I think that we should be able to change
the following in the GUI - so they must be in the database for each _fastnetmon_ instances.

```
ban_for_pps = on | off
ban_for_bandwidth = on | off
ban_for_flows = on | off

threshold_pps = bigint
threshold_mbps = bigint
threshold_flows = bigint

threshold_tcp_mbps = bigint
threshold_udp_mbps = bigint
threshold_icmp_mbps = bigint

threshold_tcp_pps = bigint
threshold_udp_pps = bigint
threshold_icmp_pps = bigint

ban_for_tcp_bandwidth = on | off
ban_for_udp_bandwidth = on | off
ban_for_icmp_bandwidth = on | off

ban_for_tcp_pps = on | off
ban_for_udp_pps = on | off
ban_for_icmp_pps = on | off
```
